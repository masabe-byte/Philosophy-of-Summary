...
function buildPrompt(payload = {}) {
  ...
  return [
    ...
    "- verdict 为 pass 且 score 达到通过门槛时，shouldAdvance 必须为 true；如果你认为不能进入下一段，verdict 必须为 revise。",
    "",
...
function normalizeResponse(raw, payload = {}) {
  ...
  const result = {
    ...
    shouldAdvance: verdict === "pass" && score >= threshold
  };
...
async function processSummaryRequest(payload = {}) {
  ...
  const content = await requestModelJson(config, [
    { role: "system", content: "Return one valid JSON object only. No markdown. Use double quotes. Ensure every array element is separated by commas." },
  ...
  try {
    ...
    if (!/JSON|Expected|Unexpected|parse/i.test(String(error?.message || ""))) throw error;
    const repaired = await requestModelJson(config, [
      { role: "system", content: "Repair malformed JSON. Return exactly one valid JSON object and nothing else." },
      {
        role: "user",
        content: [
          "下面内容本来应该是 JSON，但格式有错误。请只修复格式，不要改变字段含义。",
          "必须保留字段：ok, verdict, score, headline, feedback, missing, dimensions, revisedSummary, referenceAnswer, reason, nextAction, shouldAdvance。",
          "",
          content
        ].join("\n")
      }
    ]);
    return normalizeResponse(repaired, payload);
  }
}

async function requestModelJson(config, messages) {
  ...
  if (!response.ok) {
    if (/response_format|json_object|unsupported|not support/i.test(text)) {
      return requestModelJsonWithoutJsonMode(config, messages);
    }
    throwModelError(response.status, text);
  }
  let data;
  try {
...
async function requestModelJsonWithoutJsonMode(config, messages) {
  const response = await fetch(config.url, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
  ...
  const text = await response.text();
  if (!response.ok) throwModelError(response.status, text);
  let data;
  try {
    data = JSON.parse(text);
  } catch {
...
function throwModelError(status, text) {
  const error = new Error(`模型请求失败：${status} ${String(text || "").slice(0, 240)}`);
  error.status = 502;
  throw error;
}
